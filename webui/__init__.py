...
from manager.ProcessManager import GalluscoindManager, WalletRPCManager
...
class BaseWebUI(QMainWindow):
    def __init__(self, html, app, hub, window_size, debug=False):
        ...
        self.icon = self._getQIcon('galluscoin_icon_64.png')
...
class MainWebUI(BaseWebUI):
    def __init__(self, app, hub, debug):
        ...
        if sys.platform == 'darwin':
            tray_icon = 'galluscoin_16x16_mac.png'
        elif sys.platform == "win32":
            tray_icon = 'galluscoin_16x16.png'
        else:
            tray_icon = 'galluscoin_32x32_ubuntu.png'
        ...
        self.galluscoind_daemon_manager = None
...
#         self.view.load(qt_core.QUrl(self.url))
    ...
    def start_deamon(self):
        #start galluscoind daemon
        self.galluscoind_daemon_manager = GalluscoindManager(self.app.property("ResPath"), 
                                            self.app_settings.settings['daemon']['log_level'], 
                                            self.app_settings.settings['daemon']['block_sync_size'])
        
        self.galluscoind_daemon_manager.start()
    ...
    def run_wallet_rpc(self, wallet_password, log_level=0):
        ...
        while True:
            ...
            galluscoind_info = self.daemon_rpc_request.get_info()
            if galluscoind_info['status'] == "OK":
    ...
    def _update_daemon_status(self):
        ...
        if galluscoind_info['status'] == "OK":
            status = "Connected"
            self.current_height = int(galluscoind_info['height'])
            target_height = int(galluscoind_info['target_height'])
            ...
            status = galluscoind_info['status']
        ...
        sync_status = "Disconnected" if galluscoind_info['status'] != "OK" else "Synchronizing..."
        if galluscoind_info['status'] == "OK" and self.current_height == self.target_height:
...
#             print(json.dumps(wallet_info, indent=4))
    ...
    def about(self):
        QMessageBox.about(self, "About", \
            u"%s <br><br>Copyright© 2018 - Projek Gallus (www.galluscoin.com)" % self.agent)
    ...
    def _handleAboutToQuit(self):
        ...
        if self.wallet_rpc_manager is not None:
            self.wallet_rpc_manager.stop()
        if self.galluscoind_daemon_manager is not None:
            self.galluscoind_daemon_manager.stop()
