...
'''
Process managers for galluscoind, gallus-wallet-cli and gallus-wallet-rpc
'''
...
class GalluscoindManager(ProcessManager):
    def __init__(self, resources_path, log_level=0, block_sync_size=10):
        proc_args = u'%s/bin/galluscoind --log-level %d --block-sync-size %d' % (resources_path, log_level, block_sync_size)
        ProcessManager.__init__(self, proc_args, "galluscoind")
        self.synced = Event()
        self.stopped = Event()
...
class WalletCliManager(ProcessManager):
    ...
    def __init__(self, resources_path, wallet_file_path, wallet_log_path, restore_wallet=False, restore_height=0):
        if not restore_wallet:
            wallet_args = u'%s/bin/gallus-wallet-cli --generate-new-wallet=%s --log-file=%s' \
                                                % (resources_path, wallet_file_path, wallet_log_path)
        else:
            wallet_args = u'%s/bin/gallus-wallet-cli --log-file=%s --restore-deterministic-wallet --restore-height %d' \
                                                % (resources_path, wallet_log_path, restore_height)
        ProcessManager.__init__(self, wallet_args, "gallus-wallet-cli")
        self.ready = Event()
        self.last_error = ""
        self.block_height = 0
        self.top_height = 0
...
class WalletRPCManager(ProcessManager):
    def __init__(self, resources_path, wallet_file_path, wallet_password, app, log_level=1):
        ...
        wallet_log_path = os.path.join(os.path.dirname(wallet_file_path), "gallus-wallet-rpc.log")
        wallet_rpc_args = u'%s/bin/gallus-wallet-rpc --wallet-file %s --log-file %s --rpc-bind-port 22025 --user-agent %s --log-level %d' \
                                            % (resources_path, wallet_file_path, wallet_log_path, self.user_agent, log_level)
                                                                                
        ProcessManager.__init__(self, wallet_rpc_args, "gallus-wallet-rpc")
        sleep(0.2)
